...
comando que abre, y nada mas.
    ...
    5729:[2026-09-24 16:47:53] VUELTA 3 : APERTURA CIEGA (claude-opus-5-5), retirados: REPORTE.md ultimo_extractor.json ultimo_auditor.json CREDITO_serial.jsonl
...
fichero cuya `ACTA 65` lei entera antes de escribir esta pagina (sus lineas `46238` a `46619`).
    ...
    $ git hash-object docs/loop/ACTA_AUDITOR.md; git rev-parse 4648cbc:docs/loop/ACTA_AUDITOR.md
    24a4ab0a49ed780c45474321c09c6ca89c079a40
    24a4ab0a49ed780c45474321c09c6ca89c079a40
...
trae cifras:
    ...
    $ git log -1 --format='%h %s' c851892
    c851892 Vuelta 67, T5: el cierre (388 nodos, bandeja de Grove en 49 e insertados en 43, 11 aristas de la tanda en el grafo y 0 en cola, cap_04 0 de 143 en lo que entro, R5 en cero, guardas y cierre estricto en verde)
...
porque lo que vi es lo que hay que poder mirar:
    ...
    $ cat .v67aud/pasos_conjunta.txt
    ===== subir_productividad_gerencial_tres_vias | grafo
      titulo: Subir la productividad de un mando por sus tres vias: el ritmo, la palanca y la mezcla de actividades
      fuente: ['grove_high_output'] | previos: [] | siguientes: ['buscar_actividad_alta_palanca_tres_vias']
    ...
    ===== buscar_actividad_alta_palanca_tres_vias | grafo
      titulo: Buscar las actividades de alta palanca por sus tres vias basicas: mucha gente, un acto breve de efecto largo, y el saber unico
      fuente: ['grove_high_output'] | previos: ['subir_productividad_gerencial_tres_vias'] | siguientes: ['elegir_momento_actividad_palanca_maxima']
      ...
      P3. Segunda via: aquella en la que la actividad o la conducta de una persona durante un periodo largo de tiempo queda afectada por un conjunto de palabras o de acciones del mando que es breve y esta bien enfocado.
    ...
    ===== elegir_momento_actividad_palanca_maxima | grafo
      titulo: Elegir el momento de una actividad para que su palanca sea maxima: por delante del acontecimiento, o inmediata cuando el momento manda
      fuente: ['grove_high_output'] | previos: ['buscar_actividad_alta_palanca_tres_vias'] | siguientes: []
      ...
      P2. Haz por delante del acontecimiento el trabajo que lo prepara: define con antelacion exactamente que informacion hay que reunir y presentar en cada etapa del proceso, y deja puesto quien responde de que.
      P3. Cuenta con lo que ese trabajo hecho por delante consigue: afecta directamente al trabajo posterior de mucha gente y le quita confusion y ambiguedad a una poblacion grande durante un periodo largo.
      ...
      P5. Cuando la actividad sea de las que dependen del momento, atiendela inmediatamente: si te enteras de que un subordinado valioso ha decidido irse, dirigete a esa situacion en el acto si quieres que cambie de idea.
    ...
    ===== detectar_palanca_negativa_actividad_mando | grafo
      titulo: Detectar la palanca negativa en la propia actividad de mando: la reunion sin preparar, el desanimo, las largas y la intromision
      fuente: ['grove_high_output'] | previos: [] | siguientes: []
      ...
      P2. Empieza por el caso simple: si eres participante clave de una reunion y llegas sin preparar, no solo desperdicias el tiempo de los asistentes por tu falta de preparacion, que es el coste directo de tu descuido, sino que ademas privas a los demas participantes de la oportunidad de emplear ese tiempo en otra cosa.
      ...
      P5. Revisa si estas dando largas: dar largas es aplazar una decision que va a afectar al trabajo de otras personas, y en la practica la falta de decision es lo mismo que una decision negativa, porque ninguna luz verde es una luz roja y el trabajo puede pararse para toda una organizacion.
      P6. Cuenta con que el desanimo y las largas tienen palanca negativa practicamente ilimitada y son muy dificiles de contrarrestar, porque su efecto sobre la organizacion es a la vez omnipresente y escurridizo; en cambio, si a la gente le afecta un mal curso de formacion de ventas, la situacion se arregla volviendo a formar al grupo.
      P7. Revisa si te estas entrometiendo: hay intromision cuando un supervisor usa su conocimiento y su experiencia superiores sobre las responsabilidades de un subordinado para tomar el mando de la situacion en vez de dejar que el subordinado la resuelva por si mismo.
      P8. Aplica la prueba concreta que da el libro: si un mando superior ve un indicador que muestra una tendencia indeseable y dicta a la persona responsable un conjunto detallado de acciones a tomar, eso es intromision.
      P9. Cuenta con lo que la intromision produce: tras verse expuesto a muchos episodios asi, el subordinado pasa a tomar una vision mucho mas restringida de lo que se espera de el, muestra menos iniciativa para resolver sus propios problemas y se los lleva en cambio a su supervisor.
...
`68`.
    ...
    43
    $ ls cuarentena/grove_high_output/ | grep -E "agrupar_interrupciones|canalizar"
    agrupar_interrupciones_subordinados_reuniones_regulares.json
    canalizar_interrupciones_cartel_hora_oficina.json
    $ git diff --stat 4648cbc HEAD -- dataset/ bitacora/ censos/ config/pares_mutuos.jsonl
     bitacora/VEREDICTOS.jsonl | 97 +++++++++++++++++++++++++++++++++++++++++++++++
     censos/atribuciones.md    |  2 +
     censos/denominaciones.md  | 73 +++++++++++++++++++++++++++++++++++
     dataset/nodos.jsonl       | 28 ++++++++++++--
     4 files changed, 196 insertions(+), 4 deletions(-)
    $ git diff --name-status 4648cbc HEAD -- cuarentena/ | awk '{print $1}' | sort | uniq -c
         20 R100
    $ git diff --stat 4648cbc HEAD -- src/ config/ fuentes/ esquema/ | wc -l
    0
    $ ls -A procesos/ | wc -l
    0
...
commit. Los `P` y los `D` salen de **mi** lectura entera de `cap_04`, `.v66aud/fidelidad.tsv`, sellada en la `66`.
    ...
    $ python .v67aud/entra_lo_leido.py
    reunir_informacion_gerencial_vias_variadas             pasos  8 filas mias  8 P 0 D 0
    escalonar_fuentes_informacion_gerencial                pasos  7 filas mias  7 P 0 D 0
    programar_visita_area_observar_despachar               pasos  8 filas mias  8 P 0 D 0
    transmitir_objetivos_prioridades_preferencias          pasos  5 filas mias  5 P 0 D 0
    empujar_persona_reunion_direccion_preferida            pasos  7 filas mias  7 P 0 D 0
    subir_productividad_gerencial_tres_vias                pasos  4 filas mias  4 P 0 D 0
    buscar_actividad_alta_palanca_tres_vias                pasos  4 filas mias  4 P 0 D 1
    elegir_momento_actividad_palanca_maxima                pasos  7 filas mias  7 P 0 D 0
    detectar_palanca_negativa_actividad_mando              pasos  9 filas mias  9 P 0 D 3
    delegar_tarea_base_comun_seguimiento                   pasos 10 filas mias 10 P 0 D 0
    supervisar_tarea_delegada_etapa_menor_valor            pasos  9 filas mias  9 P 0 D 0
    supervisar_decision_delegada_preguntas_concretas       pasos  6 filas mias  6 P 0 D 0
    identificar_paso_limitante_jornada_desfases            pasos  5 filas mias  5 P 0 D 0
    agrupar_tareas_semejantes_aprovechar_preparacion       pasos  6 filas mias  6 P 0 D 0
    decir_no_trabajo_excede_capacidad                      pasos 10 filas mias 10 P 0 D 0
    usar_calendario_herramienta_planificacion_produccion   pasos  7 filas mias  7 P 0 D 0
    llevar_inventario_proyectos_discrecionales             pasos  5 filas mias  5 P 0 D 0
    dimensionar_numero_subordinados_medio_dia_semanal      pasos 11 filas mias 11 P 0 D 0
    buscar_regularidad_bloques_iguales_trabajo_mando       pasos  9 filas mias  9 P 0 D 0
    preparar_respuestas_estandar_interrupciones_repetidas  pasos  6 filas mias  6 P 0 D 0
    nodos de la tanda en el grafo iguales a su lectura entera: 20 | distintos: 0
    fichas de _insertados con el mismo blob que en d8f4e2a: 20 | distintas: 0
...
**sin las claves de relacion**:
    ...
    $ python .v67aud/poblacion.py
    en 4648cbc: 368 del grafo mas 111 de bandejas = 479 | ids distintos: 479
    hoy       : 388 del grafo mas 91 de bandejas = 479 | ids distintos: 479
    ids solo en 4648cbc: [] | ids solo hoy: []
    de la bandeja al grafo: 20
    ids con contenido distinto fuera de las claves de relacion: 0
    nodos del grafo en los dos commits: 368 | reescritos solo en claves de relacion: 4 | con otra clave cambiada: 0
...
lectura espera que la tanda deje, sacado **solo** de mis ficheros sellados de la `66` y de mi relectura de hoy:
    ...
    $ python .v67aud/esperado_67.py
    candidatos: 20 | filas de vecino de mi barrido de la 66 con candidato de los 20: 90
      ESPERADA  reunir_informacion_gerencial_vias_variadas           > escalonar_fuentes_informacion_gerencial              veredicto CONTINUA
      ESPERADA  subir_productividad_gerencial_tres_vias              > buscar_actividad_alta_palanca_tres_vias              veredicto CONTINUA
      ESPERADA  buscar_actividad_alta_palanca_tres_vias              > elegir_momento_actividad_palanca_maxima              veredicto CONTINUA
      ESPERADA  detectar_arreglar_fallo_etapa_menor_valor            > supervisar_tarea_delegada_etapa_menor_valor          veredicto CONTINUA
      QUITADA   usar_calendario_herramienta_planificacion_produccion > decir_no_trabajo_excede_capacidad                    veredicto CONTINUA | SANO, ACTA 65 65.4.b
      ESPERADA  construir_flujo_produccion_paso_limitante            > identificar_paso_limitante_jornada_desfases          por lectura
      ESPERADA  variar_frecuencia_inspeccion_nivel_calidad           > supervisar_tarea_delegada_etapa_menor_valor          por lectura
      ESPERADA  delegar_tarea_base_comun_seguimiento                 > supervisar_tarea_delegada_etapa_menor_valor          por lectura
      ESPERADA  delegar_tarea_base_comun_seguimiento                 > supervisar_decision_delegada_preguntas_concretas     por lectura
      ESPERADA  transmitir_objetivos_prioridades_preferencias        > delegar_tarea_base_comun_seguimiento                 por lectura
      ESPERADA  identificar_paso_limitante_jornada_desfases          > usar_calendario_herramienta_planificacion_produccion por lectura
      QUITADA   buscar_actividad_alta_palanca_tres_vias              > detectar_palanca_negativa_actividad_mando            por lectura        | NO, relectura de la 67
      ESPERADA  representar_actividad_caja_negra_ventanas            > buscar_regularidad_bloques_iguales_trabajo_mando     por lectura
    aristas que mi lectura espera en la tanda: 11 | quitadas por correccion declarada: 2
    esperadas por lectura: 7 | por veredicto CONTINUA: 4
    madres esperadas que ya vivian en el grafo de 4648cbc: 4 ['construir_flujo_produccion_paso_limitante', 'detectar_arreglar_fallo_etapa_menor_valor', 'representar_actividad_caja_negra_ventanas', 'variar_frecuencia_inspeccion_nivel_calidad']
