# PARADA DEL FRENTE `gerber_emyth`, 17 SEP 2026: **RESUELTA, Y SUS DOS COSAS ESTAN PAGADAS**

*Archivo de la parada de la vuelta `1` del frente, escrita por el auditor de la `ACTA G1`.
**Se archiva el 21 sep 2026 al reanudar el frente, y se archiva porque sus dos motivos
tienen respuesta escrita, no porque haya caducado.***

> ## **LA PARADA FUE BUENA LAS DOS VECES.** Levanto una pregunta que el bucle no podia
> contestar y un defecto de arnes que el bucle no podia tocar. **Las dos tenian razon.**

---

## COMO QUEDO CADA UNA

### 1. `NO SE DE QUIEN ES LA RACHA CUANDO HAY CUATRO SESIONES A LA VEZ`

**RESUELTA el mismo dia por el fundador**, y es `D.48`:
...
**Sede:** `docs/loop/paradas/2026-09-17-de-quien-es-la-racha-DECISION.md`, `D.48` del banco,
y `PARALELO.md` `3.1.c`. **El credito vive en `docs/loop/CREDITO_<linea>.jsonl`**, y el de
este frente es `CREDITO_gerber_emyth.jsonl`.

**De las dos lecturas que el auditor puso en su tabla, gano la `(b)`:** cada frente lleva la
suya, y esta tanda del frente **no arrastra el `3 de 3` de la serial.**

### 2. `UNA LINEA DEL ARNES QUE D.43 DEJO VIEJA EL 12 SEP LE ESTA COSTANDO UNA HORA A CADA FRENTE`

**ARREGLADA el 21 sep 2026, cuatro dias despues, y el retraso es de la serial y no suya.**

    antes : Al cerrar el capitulo corres el informe del lote entero y pegas su saldo en el reporte.
    hoy   : NO LANZAS EL INFORME DEL LOTE ENTERO EN TU TURNO. D.43 lo saca del turno porque NO CABE [...]

**El auditor de este frente hizo exactamente lo que debia:** la midio con instrumento
(`91` bytes de cabecera, el mismo ejemplar de `480` que motivo `D.43`), **no la toco porque
`D.45` veda el arnes desde un frente**, y la subio. **Lo que fallo es que nadie de la serial
la recogio.**

> **Y COSTO OTRA VEZ ANTES DE CAER.** La `ACTA 58` `58.9` de la serial encontro
> `.v55ext/informe_de_lote.txt` **corriendo diez horas y media por debajo de las vueltas
> `56`, `57` y `58`**, lanzado por el extractor de la vuelta `55` obedeciendo esta misma
> linea. Se anoto como `d086`, *un proceso que corrio sin que ninguna vuelta lo supiera*.
> **Era esta. `d086` queda explicado.**

**Sede del arreglo:** `orquestador_forja.sh`, con su caso positivo y su caso negativo en
`tests/test_aceptacion.py`.

---

## POR QUE EL FRENTE SE REANUDA HOY

Decision del fundador del `21` sep `2026`, punto `3`, archivada en la serial en
`docs/loop/paradas/2026-09-21-grove-consumado-DECISION.md`. **Con su enmienda del mismo dia,
`gerber_emyth` es la unica linea corriendo esta semana**: la insercion de Grove se pospone
por cuota y la serial queda parada.
