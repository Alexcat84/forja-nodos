# DECISION DEL FUNDADOR, 22 SEP 2026: **GERBER ENTRA, MARQUET SIGUE**

*Archivo de la parada por alcance del frente `gerber_emyth`, escrita por su auditor al
cerrar la `ACTA G8`. **La decision va arriba, literal. El cuerpo va debajo sin tocar una
coma.***

> ## **ES LA DECISION QUE `D.60` ESTABA ESPERANDO.** La regla dice que un sexto libro entra
> *solo si cierra su extraccion antes de que la cuota de la semana se agote*. **Gerber la
> cerro el `21` sep, y esta decision ejerce esa clausula por primera vez.**
...
> GERBER ENTRA, MARQUET SIGUE. Archivala en docs/loop/paradas/
> 2026-09-22-gerber-entra-DECISION.md y aplicala en este orden:
>
> 1. Gerber, UNA vuelta mas en su frente: cap_01, cap_02 y cap_03 se
>    minan en regimen ligero (Sonnet extractor, Opus auditor,
>    cuarentena); lo que sea ensayo se firma en cero con su motivo; d094
>    se cierra con los tres leidos y la frontera del libro completa de
>    cap_01 a cap_22. Escribe su PROMPT_SIGUIENTE y deja el comando.
> 2. Al cerrar esa vuelta: el fundador funde extraccion-gerber_emyth a
>    extraccion-mundo-11 (git merge --no-ff), corre gate, guiones y
>    suite, y el TABLERO pasa gerber a CERRADO EN EXTRACCION y COSECHADO,
>    dueño NINGUNO, con sus candidatos en bandeja esperando la insercion
>    de la semana que viene. D.60 queda cumplida y citada: Gerber es el
>    sexto libro del mundo 11.
> 3. Marquet abre como frente de extraccion bajo D.45, mismo regimen y
>    modelos, desde cap_04 con su frontera heredada, TABLERO con dueño
>    marquet. Si la cuota de la semana lo cierra, se cosecha e inserta
>    como septimo; si no, queda en bandeja entero. Deja su
>    PROMPT_SIGUIENTE y su comando.
> 4. La serial sigue parada hasta la insercion de la semana que viene,
>    que abrira con Grove (91), Gerber (22 mas los que den cap_01 a 03) y,
>    si cerro, Marquet, en ese orden y en regimen completo.
...
|---|---|
| `1` la ultima vuelta de Gerber | `PROMPT_SIGUIENTE.md` del worktree de gerber, y el comando de la seccion `COMO SE RETOMA` |
| `2` la cosecha | **es del fundador y no se hace aqui.** Lo que si se deja hecho es la declaracion del tablero preparada y la cita de `D.60` |
| `3` Marquet abre | worktree `forja-marquet_turn_the_ship`, `config/frentes.json`, `docs/loop/TABLERO.jsonl` y su `PROMPT_SIGUIENTE.md` |
| `4` el orden de la insercion | escrito en el `PARA_ALEXIS.md` de la serial, que es donde la vuelta de insercion lo va a leer |

---

## LO QUE ESTA DECISION CAMBIA EN EL ALCANCE, Y CONVIENE VERLO JUNTO

**`D.60` fijo el corte del mundo `11` en CINCO libros y dejo dos condicionales.** Esta
decision **ejerce la del sexto y abre la del septimo**:

| | libro | como queda hoy |
|---|---|---|
| `1` a `4` | `onu_consumidor`, `smart_who`, `zhuo_manager`, `scott_radical_candor` | **INSERTADOS** |
| `5` | `grove_high_output` | extraccion cerrada, `91` en bandeja. **Falta insertar** |
| **`6`** | **`gerber_emyth`** | **ENTRA.** Una vuelta mas y se cosecha |
| **`7`** | **`marquet_turn_the_ship`** | **se abre su frente.** Entra **si la cuota lo cierra** |

> **EL CORTE DE `config/frentes.json` SE ACTUALIZA CUANDO CADA UNO SE COSECHE, no hoy.**
> `D.60` dice que un mundo se cierra cuando sus libros estan INSERTADOS, y **ninguno de
> los dos lo esta.** Escribirlos en `libros_del_mundo_11` antes de tiempo seria justo lo
> que esa regla vino a impedir.

---

## LA PREMISA QUE CAYO, Y POR ESO ESTA DECISION EXISTE

`PARALELO.md` `4.d` (tu decision del `21` sep) corto a Gerber con una premisa **medida**:
que insertarlo *obligaria a minar el libro entero, y eso es lo que esta campania decide no
pagar*, **con el frente fotografiado en `10` candidatos y `4` de `22` capitulos.**

**El `21` sep por la tarde el frente cerro en `22` candidatos y `19` de `22`.** El coste que
la decision del `21` decidio no pagar **se pago entre sus vueltas `4` y `8`**.

**No cayo la decision: cayo su premisa**, y esta decision es la que la sustituye.
