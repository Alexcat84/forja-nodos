# -*- coding: utf-8 -*-
"""COPIA DE LA VUELTA 66 de .v64ext/comprobar_veredictos.py, con la fuente de los vecinos cambiada: en vez de
.v64ext/pares_despues.txt y las matrices de la 64, lee el barrido de hoy, .v66ext/vecinos_<id>.json (TAREA 3.2).
Comprueba .v66ext/veredictos_listos.txt contra dos cosas, sin tocar nada:
...
SOSTENGO de .v66ext/aristas_lectura.txt, si el barrido de hoy levanta el par en algun sentido (deberia salir NO)."""
...
for f in sorted(glob.glob('.v66ext/vecinos_*.json')):
    d = json.load(io.open(f, encoding='utf-8'))
    hoy[d['id']] = set(v['id'] for v in d['vecinos'])
los22 = [l.split()[0] for l in io.open('.v66ext/los22_cap04.txt', encoding='utf-8') if l.strip() and not l.startswith('#')]
