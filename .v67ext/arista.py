...
COPIA DE LA VUELTA 67 de .v65ext/arista.py, con la sede cambiada a .v66ext/aristas_lectura.txt y la cita a la ACTA 65.
    python .v67ext/arista.py <madre> <hijo> <paso_de_la_madre>
La razon es la de su fila, tal cual. --veredicto CONTINUA y --cita-veredicto a la fila y a la ACTA 65 (D65.2).
Guarda la salida en .v67ext/arista_<madre>__<hijo>.txt."""
...
cita = ('leida CONTINUA: .v66ext/aristas_lectura.txt linea %d (%s; %s), sostenida por la ACTA 65 seccion 65.4.b; '
        'cableada en la vuelta 67 con la madre ya en el grafo' % (n, c[3], c[4]))
...
out = out.replace('\r\n', '\n')
io.open('.v67ext/arista_%s__%s.txt' % (madre, hijo), 'w', encoding='utf-8', newline='\n').write(
