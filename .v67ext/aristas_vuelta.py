# -*- coding: utf-8 -*-
"""COPIA DE LA VUELTA 67 de .v65ext/aristas_vuelta.py (encargo, TAREA 5): las aristas que la vuelta 67 dejo en
bitacora/VEREDICTOS.jsonl (lineas 797 en adelante; la 796 era la ultima al abrir, 67.0), y las que siguen EN COLA
porque un extremo espera en la bandeja. Comprueba que cada arista cableada vive en el grafo (madre con el hijo en
nodos_siguientes), y la cruza con las ESPERADAS: las CONTINUA con madre= de las filas 1 a 20 en
.v66ext/veredictos_listos.txt y las SOSTENGO de .v66ext/aristas_lectura.txt cuyo hijo esta en las filas 1 a 20."""
import io, json, re
L = [json.loads(l) for l in io.open('bitacora/VEREDICTOS.jsonl', encoding='utf-8') if l.strip()][796:]
g = dict((d['id'], d) for d in (json.loads(l) for l in io.open('dataset/nodos.jsonl', encoding='utf-8') if l.strip()))
tanda = [l.split()[1] for l in io.open('.v66ext/orden.txt', encoding='utf-8') if re.match(r'^\d+\s', l)][0:20]
