...
@@RUN:0::grep -c -E "previos|siguientes" .v68aud/pasos_cap05.txt .v68aud/pasos_cap06.txt .v68aud/pasos_fuera_a.txt .v68aud/pasos_fuera_b.txt .v68aud/pasos_pares.txt@@
@@RUN:0::grep -l -E "nodos_previos|nodos_siguientes|previos|siguientes" .v68aud/*.py | wc -l@@
...
@@RUN:0::git diff --stat 587f1d8 3d28595 -- dataset/ bitacora/ censos/ config/pares_mutuos.jsonl@@
@@RUN:0::git diff --name-status 587f1d8 3d28595 -- cuarentena/@@
...
@@RUN:0::python .v68aud/cobertura.py@@
...
@@RUN:0::python .v68aud/entra_lo_leido.py@@
@@RUN:0::python .v68aud/entra_lo_leido.py 22 | tail -5@@
...
@@RUN:0::grep -E "agrupar_interrupciones|canalizar_interrupciones" .v66aud/aristas_lectura.tsv .v66aud/mis_clases.tsv | cut -f1-4@@
...
@@RUN:0::wc -l fuentes/grove_high_output/cap_05.md fuentes/grove_high_output/cap_06.md@@
...
@@RUN:0::python .v68aud/contar_fidelidad.py@@
...
@@RUN:0::head -1 .v68aud/barrido.log; tail -1 .v68aud/barrido.log; grep -c "rc=0" .v68aud/barrido.log; grep -c "rc=" .v68aud/barrido.log@@
@@RUN:0::sha1sum -c --quiet .v68aud/huellas_al_barrer.txt && echo "bandeja de grove, las dos filas y el grafo: mismas huellas que al barrer"@@
@@RUN:0::cat .v68aud/head_al_barrer.txt; git rev-parse HEAD@@
...
@@RUN:0::cat .v68aud/barrido_abortado.log@@
@@RUN:0::sed -n '24p;43p' .v66aud/cola_grove.txt; head -1 .v68aud/los20.txt; tail -1 .v68aud/los20.txt@@

**Poblacion y vecinos por candidato:**

@@RUN:30:.v68aud/vecinos_tabla.txt:python .v68aud/vecinos_tabla.py | tee .v68aud/vecinos_tabla.txt | sed -n '1,/^sin fichero/p'@@
@@RUN:0::grep -c ">" .v68aud/vecinos_tabla.txt@@
@@RUN:0::grep ">" .v68aud/vecinos_tabla.txt | grep -c " grafo "@@
@@RUN:0::tail -1 .v68aud/vecinos_tabla.txt@@

**Las filas de vecino de los `20`, sin las dos filas de `cap_04`, y los pares sin orden que tocan a los `20`:**

@@RUN:0::grep ">" .v68aud/vecinos_tabla.txt | grep -Ev "^    (agrupar_interrupciones|canalizar_interrupciones)" | wc -l@@
@@RUN:0::grep "~" .v68aud/vecinos_tabla.txt | grep -Ev "agrupar_interrupciones_subordinados|canalizar_interrupciones_cartel" | awk '{print $4}' | sort | uniq -c@@
@@RUN:0::grep ">" .v68aud/vecinos_tabla.txt | grep -Ev "^    (agrupar_interrupciones|canalizar_interrupciones)" | grep " grafo "@@

**Y las dos filas de `cap_04` contra mi barrido sellado de la `66`:**

@@RUN:0::python .v68aud/filas_66_68.py@@
...
@@RUN:0::grep "rc=" .v68aud/barrido.log | sed 's/.*segundos=//' | sort -n | sed -n '1p;$p'@@
...
@@RUN:0::python .v68aud/cruce_clases.py@@
...
@@RUN:0::python .v68aud/armar_clases.py@@
@@RUN:0::grep -v "^[#@]" .v68aud/clases_intra.txt | wc -l@@
...
@@RUN:0::python .v68aud/cruce_aristas.py@@
...
@@RUN:0::python .v68aud/d37_partes.py@@
...
@@RUN:0::python .v68aud/restricciones_orden.py@@
