# -*- coding: utf-8 -*-
"""Barrido D.38.4 de la fase ciega de la 65 para UN insertado: su ficha de
cuarentena/_insertados/grove_high_output/ (la que entro), normalizada como la
...
No escribe nada fuera de .v65aud/."""
import io, json, sys
sys.path.insert(0, '.')
from src import aduana, comun, config as modulo_config
i = sys.argv[1]
bruto = comun.leer_json('cuarentena/_insertados/grove_high_output/%s.json' % i)
...
json.dump(salida, io.open('.v65aud/vecinos_%s.json' % i, 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
