...
acta (`658d4018`), con quien lo escribio; las `45` de fila van aparte, por su asunto:
    ...
    45
    $ git diff --name-status 658d4018 a70bdf05 | grep -v "^[AM].\.v77ext/" | sed "s|/[^/]*\.json$|/*.json|" | cut -f1,2 | sort | uniq -c
...
queda sin commitear en el dato. Las dos lineas de `DEUDA.jsonl`, y ninguna borrada:
    ...
    $ git diff 658d4018 a70bdf05 -- docs/loop/DEUDA.jsonl | grep -c "^-{"; git diff 658d4018 a70bdf05 -- docs/loop/DEUDA.jsonl | grep "^+{" | grep -o "\"id\": \"d[0-9]*\", \"linea\": \"[a-z]*\", \"tipo\": \"[a-z]*\", \"vuelta\": \"[0-9]*\""
    0
...
carpeta fuera del repositorio:
    ...
    $ cat .v77aud/normal/reproduce.txt
    ...
    pasos_y_huellas.py de hoy: IDENTICO a la de la 76 salvo la sede
    orden.py corrido sobre el arbol de d967b0e4 (T3, antes del primer insertar): IDENTICO a .v77ext/orden.txt

(El fichero guarda tambien el intento de correr `orden.py` sobre el arbol de hoy, que cae con `FileNotFoundError` por la bandeja vacia,
y las lineas de aristas esperadas de `.v77ext/orden.txt`; lo de arriba son sus lineas de resultado.) **Las huellas de hoy de las
...
bitacora de cada fila en el ultimo commit de su fila, contra la cifra *Bitacora de X a Y* de su nota**:

    $ bash .v77aud/normal/por_fila.sh | awk '{print $2, $6, $7, $8, $9, $10, $11, $12, $13, $14}' | tr '\n' ';'
    1 bitacora 1112 | el reporte: Bitacora de 1111 a 1112;2 bitacora 1120 | el reporte: Bitacora de 1112 a 1120;3 bitacora 1121 | el reporte: Bitacora de 1120 a 1121;4 bitacora 1122 | el reporte: Bitacora de 1121 a 1122;5 bitacora 1122 | el reporte: Bitacora sin movimiento: 1122 ;6 bitacora 1126 | el reporte: Bitacora de 1122 a 1126;7 bitacora 1127 | el reporte: Bitacora de 1126 a 1127;8 bitacora 1132 | el reporte: Bitacora de 1127 a 1132;9 bitacora 1135 | el reporte: Bitacora de 1132 a 1135;10 bitacora 1141 | el reporte: Bitacora de 1135 a 1141;11 bitacora 1144 | el reporte: Bitacora de 1141 a 1144;12 bitacora 1149 | el reporte: Bitacora de 1144 a 1149;13 bitacora 1151 | el reporte: Bitacora de 1149 a 1151;14 bitacora 1152 | el reporte: Bitacora de 1151 a 1152;15 bitacora 1154 | el reporte: Bitacora de 1152 a 1154;16 bitacora 1157 | el reporte: Bitacora de 1154 a 1157;17 bitacora 1159 | el reporte: Bitacora de 1157 a 1159;18 bitacora 1161 | el reporte: Bitacora de 1159 a 1161;19 bitacora 1166 | el reporte: Bitacora de 1161 a 1166;20 bitacora 1170 | el reporte: Bitacora de 1166 a 1170;21 bitacora 1170 | el reporte: Bitacora sin movimiento: 1170 ;22 bitacora 1172 | el reporte: Bitacora de 1170 a 1172;
...
- **Los `22` entraron en el orden de `.v76ext/orden.txt`**, que es el de mi encargo (`APERTURA_CIEGA.md` `4`), **uno por vez**: cada uno
  ...
  *que ya no levantan: 0*, y las lineas son las del barrido (bloque de arriba). **La bitacora de cada fila es la de su nota.**
...
entro es byte a byte lo que lei; lo mido otra vez hoy contra el grafo:
    ...
    $ python .v77aud/entra_lo_leido.py | cut -c1-150
...
Al abrir, y despues de anotar mi tanda:
    ...
    $ cat .v77aud/normal/credito_cerrar.txt
      especie            racha      de donde sale
      ----------------------------------------------------------------------
...
por eso, despues de esa. **Sin bloqueante.** La frase de *continuar desde `cap_17`* es de extraccion y no aplica.
