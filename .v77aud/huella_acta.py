# -*- coding: utf-8 -*-
"""Fase ciega de la 77 (copia de .v75aud/huella_acta.py con la acta cambiada a la 75), sin git: la huella que el prompt me entrega (ACTA ANTERIOR LEIDA) contra el blob del fichero
...
ACTA 75, que es la que lei entera. Solo lee."""
import hashlib, sys
...
ini = [n + 1 for n, l in enumerate(ls) if l.startswith('# ACTA 75.')]
print('lineas del fichero: %d | la ACTA 75 empieza en la linea: %s' % (len(ls) - (1 if ls[-1] == '' else 0), ini))
