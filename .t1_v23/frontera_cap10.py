# -*- coding: utf-8 -*-
"""LA FRONTERA DE cap_10 RECERRADA EN LA VUELTA 23, CON EL TRAMO L88 A L92 DENTRO
DE LA PIEZA 14 (correccion 3 de la cola del auditor, ACTA 22 seccion 3.7).

No copio la tabla de la vuelta 22: la vuelvo a cerrar contra el cuerpo, porque la
pieza 14 saca AHORA CUATRO tramos del resto y no tres. Si la suma no cierra, no se
publica ninguna cuenta de nodos (ACTA 18 seccion 7.5 orden 1).

LO QUE CAMBIA CONTRA LA VUELTA 22, y es todo lo que cambia: el tramo L88 a L92
(101 palabras) deja de ser resto y pasa a ser el tramo d de la pieza 14. LA
FRONTERA SIGUE EN 14 PIEZAS: el tramo d aporta 0 nodos porque la pieza 14 ya esta
contada en su tramo c.
"""
import json
import unicodedata
...
def primera(desde, hasta=None):
    # LA PRIMERA LINEA CON CONTENIDO DEL TRAMO, no la primera a secas. El tramo d
    # de la pieza 14 empieza en L88, que es la linea de separacion `* * *`, y una
    # celda de prueba que imprime una raya no prueba nada.
    for i in range(desde, (hasta or desde) + 1):
        if LINEAS[i - 1].strip() and LINEAS[i - 1].strip() != '* * *':
            return '%d:%s' % (i, llana(LINEAS[i - 1])[:78])
...
TRAMOS = [
    ...
    ('L88 a L92',   88,  92, 0, 'pieza 14, tramo d: lo que se juega en hacerlas bien (correccion 3)'),
...
print('la pieza 14, sus cuatro tramos         : %d palabras' % (palabras(19, 19) + palabras(21, 21) + palabras(43, 43) + palabras(88, 92)))
