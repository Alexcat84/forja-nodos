# -*- coding: utf-8 -*-
"""Fase ciega de la 74 (copia de .v73aud/huella_acta.py con la acta cambiada), sin git: la huella que el prompt me entrega (ACTA ANTERIOR LEIDA) contra el blob del fichero
...
ACTA 72, que es la que lei entera. Solo lee."""
import hashlib, sys
...
ini = [n + 1 for n, l in enumerate(ls) if l.startswith('# ACTA 72.')]
print('lineas del fichero: %d | la ACTA 72 empieza en la linea: %s' % (len(ls) - (1 if ls[-1] == '' else 0), ini))
