...
# TU TURNO ACABA CUANDO TU TRABAJO ACABA (23 sep 2026, ESPECIE ARNES). Va al final de
# los TRES prompts: extractor, auditor ciego y auditor.
#
# EL 23 SEP TRES ASIENTOS CERRARON SU TURNO ESPERANDO UN TRABAJO DE FONDO QUE NADIE
# RECOGIO: el extractor de la serial con una insercion ("I'll pick up as soon as the
# insertion 1 result lands"), el auditor ciego de la serial con 16 informes que quedaron
# en 0 bytes, y el extractor de marquet con una aduana ("I'll pause here and wait for
# the background check to finish"). El de la insercion ya lo prohibe su mandato; este
# es el mismo defecto en los demas asientos.
#
# NO PROHIBE TRABAJAR EN PARALELO, y es a proposito: el auditor ciego de la serial lanzo
# 22 aduanas a la vez esa misma manana Y SE QUEDO VIGILANDOLAS DENTRO DE SU TURNO. Eso
# esta bien. Lo que esta mal es CERRAR el turno con algo vivo.
REGLA_DEL_TURNO="TU TURNO ACABA CUANDO TU TRABAJO ACABA. Puedes lanzar trabajos de fondo o en paralelo, pero NUNCA termines tu turno con uno vivo: recogelos todos dentro del turno, vigilandolos si tardan. Un proceso que sobrevive a tu turno lo mata el cierre del turno y nadie lo recoge: el 23 sep tres asientos cerraron diciendo que esperaban un trabajo de fondo, y ninguno volvio. Si algo no cabe en tu turno, NO lo lances: dilo en tu reporte o en tu acta."

PROMPT_EXTRACTOR_BASE="Estas en el repo forja-nodos. Lee docs/loop/EXTRACTOR.md (tus reglas permanentes) y despues docs/loop/PROMPT_SIGUIENTE.md (tu encargo). Ejecuta el encargo al pie de la letra. $MANDATO_INSERCION Abre docs/loop/REPORTE.md al empezar y hazlo crecer por anexion, con los discutibles marcados antes de saber si aciertas. Commitea y pushea TODO a la rama activa antes de terminar. $REGLA_DEL_TURNO"
