# -*- coding: utf-8 -*-
"""COPIA DE LA VUELTA 78 de .v76ext/orden.py (TAREA 4.3 del encargo de la 78). Lo cambiado, y nada mas: las rutas de .v76ext a
.v78ext; la lista, que es .v78ext/lista_barrido.txt (la misma forma); la bandeja de la que se lee la PIEZA, que es
cuarentena/marquet_turn_the_ship/; ORDEN, que es la propuesta de la 78; TOPE, que es 20; el patron de la PIEZA, que en Marquet se
escribe a veces en minuscula (la pieza P1) y una vez como pieza combinada P1a mas P1b, asi que se lee sin distinguir mayusculas y con
una letra final opcional; y el rotulo de las aristas esperadas, que es de la vuelta que inserte estas 20. Lo que decia la de la 76:
COPIA DE LA VUELTA 76 de .v73ext/orden.py (TAREA 4.3 del encargo de la 76). Lo cambiado, y nada mas: las rutas de .v73ext a
...
ORDEN = [
    ...
    'cambiar_forma_trabajar_conservar_plantilla',
    ...
    'contar_firmas_cadena_tramite_parado',
    'inspeccionar_reparto_informacion_notas_jefe',
    'recorrer_organizacion_escuchar_plantilla',
    ...
    'asignar_responsable_unico_evolucion_planificada',
    'reforzar_principios_guia_lenguaje_prueba_conocimiento',
    'declarar_intencion_reemplazar_peticion_permiso',
    ...
    'tomar_accion_deliberada_pausar_vocalizar_gesticular',
...
import re
def _pieza(i):
    r = json.load(io.open('cuarentena/marquet_turn_the_ship/%s.json' % i, encoding='utf-8'))['resumen_teorico']
    m = re.search(r'(?i)piezas? (?:combinada )?([A-Z][0-9]+[a-z]?|COMPUESTA)', r)
    return m.group(1) if m else '?'
L22 = collections.OrderedDict((l.split()[1], (l.split()[0], _pieza(l.split()[1])))
                              for l in io.open('.v78ext/lista_barrido.txt', encoding='utf-8') if l.strip() and not l.startswith('#'))
assert sorted(ORDEN) == sorted(L22), 'ORDEN no son las 20'
# madres
...
for i in L22:
    d = json.load(io.open('.v78ext/vecinos_%s.json' % i, encoding='utf-8'))
...
print()
print('ARISTAS ESPERADAS EN LA VUELTA QUE INSERTE ESTAS 20')
cont = set()
act = None
for l in io.open('.v78ext/veredictos_listos.txt', encoding='utf-8'):
    l = l.rstrip(chr(10))
