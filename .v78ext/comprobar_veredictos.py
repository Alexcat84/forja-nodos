# -*- coding: utf-8 -*-
"""COPIA DE LA VUELTA 78 de .v76ext/comprobar_veredictos.py, con las rutas cambiadas de .v76ext a .v78ext y la lista a las 20
de .v78ext/lista_barrido.txt (misma forma: capitulo, id, la palabra pasos y su cuenta). Nada mas cambiado. Lo que decia la de la 76:
COPIA DE LA VUELTA 76 de .v73ext/comprobar_veredictos.py, con las rutas cambiadas de .v73ext a .v76ext y la lista a
las 22 de .v76ext/lista_barrido.txt (capitulo, id, pasos y su cuenta; el id es la segunda columna, como en la 73). Nada mas
cambiado. Lo que decia la de la 73: COPIA DE LA VUELTA 73 de .v71ext/comprobar_veredictos.py, con las rutas cambiadas de .v71ext a .v73ext y la lista a las 7
...
for f in sorted(glob.glob('.v78ext/vecinos_*.json')):
    d = json.load(io.open(f, encoding='utf-8'))
    hoy[d['id']] = set(v['id'] for v in d['vecinos'])
los22 = [l.split()[1] for l in io.open('.v78ext/lista_barrido.txt', encoding='utf-8') if l.strip() and not l.startswith('#')]
