...
def cargar(idn):
    ...
    return None, None
N = int(sys.argv[1])
for idn in sys.argv[2:]:
    d, sede = cargar(idn)
    if d is None: print("NO ENCONTRADO:", idn); continue
    print("="*100)
    print(idn, "[", sede, "] fuentes:", [f.get("clave") if isinstance(f,dict) else f for f in d.get("fuentes",[])])
    print("  CONDICION:", (d.get("condiciones_activacion") or "")[:300])
    print("  ENTREGABLE:", (d.get("entregable_esperado") or "")[:300])
