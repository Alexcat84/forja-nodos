#!/bin/bash
# COPIA DE LA VUELTA 76 de .v73ext/citas.sh, con la ruta cambiada a .v76ext/fidelidad.tsv y a fuentes/gerber_emyth/, y UNA cosa mas,
...
while IFS='|' read -r id paso marca linea nota; do
  ...
  id=$(echo $id)
  case "$linea" in
    cap_*) cap="fuentes/gerber_emyth/${linea%%:*}.md"; l=$(echo ${linea#*:} | tr -d 'L ');;
    *) l=$(echo $linea | tr -d 'L '); cap=$(grep -o "fuentes/gerber_emyth/cap_[0-9][0-9]\.md" cuarentena/gerber_emyth/$id.json | head -1);;
  esac
...
done < <(grep -v '^#' .v76ext/fidelidad.tsv)
