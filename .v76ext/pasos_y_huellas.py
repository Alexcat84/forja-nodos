# -*- coding: utf-8 -*-
"""COPIA DE LA VUELTA 76 de .v73ext/pasos_y_huellas.py (encargo de la 76, TAREA 5). Lo cambiado, y nada mas: la lista,
...
ruta = lambda i: 'cuarentena/gerber_emyth/%s.json' % i
hoy = lambda i: ruta(i) if subprocess.run(['git', 'cat-file', '-e', 'HEAD:' + ruta(i)], stderr=subprocess.DEVNULL).returncode == 0 else 'cuarentena/_insertados/gerber_emyth/%s.json' % i
filas = [l.split() for l in io.open('.v76ext/orden.txt', encoding='utf-8') if l[:1].isdigit()]
lista = [f[1] for f in filas if 1 <= int(f[0]) <= 22]
...
print('fichas de las filas 1 a 22: %d | pasos: %d | iguales a su blob en %s: %d | distintas: %d | fichero de trabajo distinto de HEAD: %d' % (
