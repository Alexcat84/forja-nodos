...
actual = None
for l in io.open('.v76ext/veredictos_listos.txt', encoding='utf-8'):
    l = l.rstrip('\n')
    if l.startswith('## '):
...
for f in sorted(glob.glob('.v76ext/vecinos_*.json')):
    d = json.load(io.open(f, encoding='utf-8'))
    hoy[d['id']] = set(v['id'] for v in d['vecinos'])
los22 = [l.split()[1] for l in io.open('.v76ext/lista_barrido.txt', encoding='utf-8') if l.strip() and not l.startswith('#')]
...
for l in io.open('.v76ext/aristas_lectura.txt', encoding='utf-8'):
