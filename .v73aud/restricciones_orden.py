# -*- coding: utf-8 -*-
"""Fase ciega de la 73 (copia de .v71aud/restricciones_orden.py con la tanda cambiada): las restricciones de orden que MI
lectura pone a la insercion de las 7, escritas antes de ver el orden del extractor. (a) madre antes que hijo (D.29, D.36)
por mis CONTINUA de .v73aud/mis_clases.tsv y mis SOSTENGO de .v73aud/aristas_lectura.tsv con los dos extremos en la tanda;
...
.v73aud/los7.txt (esa lista va por capitulo y alfabetica): lo tomo por capitulo y primera linea de mi fidelidad
(.v73aud/fidelidad_fuente.txt). Solo lee."""
...
for f in [l.rstrip('\n').split('\t') for l in io.open('.v73aud/mis_clases.tsv', encoding='utf-8')][1:]:
