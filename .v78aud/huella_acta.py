# -*- coding: utf-8 -*-
"""Fase ciega de la 78 (copia de .v76aud/huella_acta.py con la acta cambiada), sin git: la huella que el prompt me entrega (ACTA ANTERIOR LEIDA) contra el blob del fichero
...
ACTA 76, que es la que lei entera. Solo lee."""
import hashlib, sys
...
ini = [n + 1 for n, l in enumerate(ls) if l.startswith('# ACTA 76.')]
print('lineas del fichero: %d | la ACTA 76 empieza en la linea: %s' % (len(ls) - (1 if ls[-1] == '' else 0), ini))
