# -*- coding: utf-8 -*-
"""Fase ciega de la 78 (copia de .v76aud/restricciones_orden.py con la tanda cambiada): las restricciones de orden que MI
lectura pone a la insercion de las 20, escritas antes de ver el orden del extractor. (a) madre antes que hijo (D.29, D.36)
por mis CONTINUA de .v78aud/mis_clases.tsv y mis SOSTENGO de .v78aud/aristas_lectura.tsv con los dos extremos en la tanda;
...
.v78aud/las20.txt (esa lista va alfabetica): lo tomo por capitulo y primera linea de mi fidelidad
(.v78aud/fidelidad_fuente.txt). Solo lee."""
...
for l in io.open('.v78aud/fidelidad_fuente.txt', encoding='utf-8'):
    if l.strip():
...
for f in [l.rstrip('\n').split('\t') for l in io.open('.v78aud/mis_clases.tsv', encoding='utf-8')][1:]:
