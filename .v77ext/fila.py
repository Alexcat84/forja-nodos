# -*- coding: utf-8 -*-
"""Talla la fila del reporte de un insertar de la vuelta 77 desde su salida guardada (COPIA DE LA VUELTA 77 de .v75ext/fila.py, ruta cambiada; la de la 75 era COPIA DE LA VUELTA 75 de .v72ext/fila.py, ruta cambiada; la de la 72 era COPIA de .v70ext/fila.py).
    python .v77ext/fila.py <fila> <id> <commit_de_insercion>"""
import io, re, sys
fila, cid, commit = sys.argv[1:4]
t = io.open('.v77ext/insertar_%s_%s.txt' % (fila, cid), encoding='utf-8').read()
...
print('')
print('La aduana de hoy: **%s** con `%d` vecino(s) contra `%s`; lineas `--veredicto` pasadas: `%d`. Salida entera en `.v77ext/insertar_%s_%s.txt`.' % (
    estado, len(vec), re.search(r'blocking multi señal contra (\d+)', t).group(1), len(lin), fila, cid))
print('')
print('<!-- TALLADO: parcial salida=.v77ext/insertar_%s_%s.txt -->' % (fila, cid))
print('')
