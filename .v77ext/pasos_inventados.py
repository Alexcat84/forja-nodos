# -*- coding: utf-8 -*-
"""COPIA DE LA VUELTA 77 de .v75ext/pasos_inventados.py (encargo de la 77, TAREA 5). Lo cambiado, y nada mas: la lectura, que es .v76ext/fidelidad.tsv (la de .v76ext/contar_fidelidad.txt, que la ACTA 75 75.3 firmo con sus PUENTE corregidos en la ficha y 0 que entran); la base, 2407dbb, la apertura de la 76, antes de su TAREA 2 que corrigio los P; la bandeja, gerber_emyth; y la tanda, las filas 1 a 22 de .v76ext/orden.txt. Lo que decia la de la 75: COPIA DE LA VUELTA 75 (encargo de la 75, TAREA 5): la lectura es .v73ext/fidelidad.tsv, la de .v73ext/contar_fidelidad.txt, que la ACTA 72 72.4 firmo con sus PUENTE corregidos en la ficha y 0 que entran; la base de comparacion es 4318e81, la apertura de la 73, antes de su TAREA 2 que corrigio los P; la tanda, las filas 1 a 7 de .v73ext/orden.txt. Lo que decia la de la 72: COPIA DE LA VUELTA 72 de .v70ext/pasos_inventados.py (encargo de la 72, TAREA 5): la tanda es la de las filas 1 a 20 de
...
def tsv(ruta):
    ...
    return m
lect = tsv('.v76ext/fidelidad.tsv')
import subprocess
BASE = '2407dbb'
viejo = lambda i: json.loads(subprocess.run(['git', 'show', '%s:cuarentena/gerber_emyth/%s.json' % (BASE, i)], capture_output=True, text=True, encoding='utf-8').stdout)['pasos_accionables']
...
entro = [i for i in tanda if os.path.exists('cuarentena/_insertados/gerber_emyth/%s.json' % i) and i in grafo]
tot = collections.OrderedDict(); faltan = []
print('%-60s %-7s %5s %3s %3s %4s' % ('candidato que ENTRO', 'cap', 'pasos', 'T', 'P', 'corr'))
for i in entro:
    d = json.load(io.open('cuarentena/_insertados/gerber_emyth/%s.json' % i, encoding='utf-8'))
    n = len(d['pasos_accionables'])
    cap = re.search(r'fuentes/gerber_emyth/(cap_\d+)\.md', d['resumen_teorico']).group(1)
    m = lect.get(i, {})
