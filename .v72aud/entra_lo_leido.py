# -*- coding: utf-8 -*-
"""Fase ciega de la 72, sin git (copia de .v70aud/entra_lo_leido.py con la tanda de la 71): para las 20 de
.v71aud/los20.txt, (1) donde estan hoy: grafo, bandeja de Grove, _insertados; (2) que el nodo del grafo trae lo que se
leyo: titulo, condiciones, pasos, entregable y resumen, contra su ficha de _insertados (cuya huella es la de mi barrido
de la 71, .v72aud/huellas_hoy.py); (3) PASOS INVENTADOS de lo que ENTRO, por capitulo (8.2), desde MI lectura sellada
de la 71 (.v71aud/fidelidad_fuente.txt, una fila T, P o D por paso), con su suma (R7). Las 6 D las adjudique T en la
ACTA 70 70.4. El capitulo de cada una sale de .v70aud/normal/bandeja_grove.txt. NO imprime ninguna clave de relacion
(R6). Solo lee."""
...
for k in sorted(tot):
    ...
    print('%s lo que ENTRO: candidatos %d | pasos %d | mis marcas: %s | suma: %d | PUENTE %d de %d = %.2f por ciento | con las D adjudicadas T (ACTA 70 70.4): T %d, P %d, suma %d' % (
        k, t['cand'], t['pasos'], m, sum(m.values()), t['P'], t['pasos'], 100.0 * t['P'] / t['pasos'], t['T'] + t['D'], t['P'], t['T'] + t['D'] + t['P']))
m = dict((c, T[c]) for c in ('T', 'P', 'D'))
print('los seis: candidatos %d | pasos %d | mis marcas: %s | suma: %d | PUENTE %d de %d' % (T['cand'], T['pasos'], m, sum(m.values()), T['P'], T['pasos']))
