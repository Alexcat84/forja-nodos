# -*- coding: utf-8 -*-
"""LA IDENTIDAD DE LA VUELTA 1 DEL FRENTE, LEIDA DE GIT Y NO TECLEADA (EXTRACTOR.md 5)."""
import subprocess
...
APERTURA = "be2b678"      # el commit del loop.log pendiente, que es mi primera operacion
commits = git("rev-list", "--count", APERTURA + "..HEAD")

print("| pieza | valor |")
print("|---|---|")
print("| rama | `%s` |" % git("rev-parse", "--abbrev-ref", "HEAD"))
print("| worktree | `%s` |" % git("rev-parse", "--show-toplevel").split("/")[-1])
print("| commit de apertura de la vuelta | `%s` |" % APERTURA)
