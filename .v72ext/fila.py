# -*- coding: utf-8 -*-
"""Talla la fila del reporte de un insertar de la vuelta 72 desde su salida guardada (COPIA DE LA VUELTA 72 de .v70ext/fila.py, ruta cambiada; la de la 70 era COPIA de .v68ext/fila.py).
    python .v72ext/fila.py <fila> <id> <commit_de_insercion>"""
import io, re, sys
fila, cid, commit = sys.argv[1:4]
t = io.open('.v72ext/insertar_%s_%s.txt' % (fila, cid), encoding='utf-8').read()
...
print('')
print('La aduana de hoy: **%s** con `%d` vecino(s) contra `%s`; lineas `--veredicto` pasadas: `%d`. Salida entera en `.v72ext/insertar_%s_%s.txt`.' % (
    estado, len(vec), re.search(r'blocking multi señal contra (\d+)', t).group(1), len(lin), fila, cid))
print('')
print('<!-- TALLADO: parcial salida=.v72ext/insertar_%s_%s.txt -->' % (fila, cid))
print('')
