...
# el bloque G3 de esta vuelta, contra docs/loop/REPORTE.md. Toda referencia G3.x que el
...
ini = max(i for i, l in enumerate(texto) if l.startswith('# FRENTE `gerber_emyth`, VUELTA 3'))
bloque = texto[ini:]
print("el bloque del frente empieza en la linea", ini + 1, "de docs/loop/REPORTE.md")
encabezados = set()
for l in bloque:
    m = re.match(r'^#{2,4}\s+(G3\.[0-9]+(?:\.[a-z])?)\.', l)
    if m: encabezados.add(m.group(1))
refs = {}
for n, l in enumerate(bloque):
    for m in re.finditer(r'`(G3\.[0-9]+(?:\.[a-z])?)`', l):
