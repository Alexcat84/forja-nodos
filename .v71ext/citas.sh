#!/bin/bash
# COPIA DE LA VUELTA 71 de .v68ext/citas.sh. Lo cambiado en la 71: la ruta a .v71ext/fidelidad.tsv y el capitulo leido de la
# UNIDAD DE ORIGEN de su ficha con cualquier numero de dos cifras (cap_07 a cap_14). Lo que ya traia la de la 68:
...
while IFS='|' read -r id paso marca linea nota; do
  ...
  id=$(echo $id); cap=$(grep -o "fuentes/grove_high_output/cap_[0-9][0-9]\.md" cuarentena/grove_high_output/$id.json | head -1)
...
done < <(grep -v '^#' .v71ext/fidelidad.tsv)
