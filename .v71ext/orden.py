# -*- coding: utf-8 -*-
"""COPIA DE LA VUELTA 71 de .v69ext/orden.py (TAREA 4.4 del encargo de la 71). Lo cambiado, y nada mas: las rutas de .v68ext a
...
ORDEN = [
    ...
    'fijar_periodo_direccion_objetivos_retroalimentacion',
    'fijar_horizonte_ventana_replanificacion',
    'definir_entorno_grupo_clientes_proveedores_competidores',
    'examinar_entorno_expectativas_tecnologia_proveedores_grupos',
    'examinar_demanda_entorno_dos_marcos_temporales',
    'determinar_estado_presente_capacidades_proyectos_merma',
    'cerrar_brecha_dos_preguntas_estrategia',
    'contestar_dos_preguntas_direccion_objetivos',
...
import re
def _pieza(i):
    r = json.load(io.open('cuarentena/grove_high_output/%s.json' % i, encoding='utf-8'))['resumen_teorico']
    m = re.search(r'PIEZAS? (P[0-9]+|COMPUESTA)', r)
    return m.group(1) if m else '?'
L22 = collections.OrderedDict((l.split()[1], (l.split()[0], _pieza(l.split()[1])))
                              for l in io.open('.v71ext/los20.txt', encoding='utf-8') if l.strip() and not l.startswith('#'))
...
for i in L22:
    d = json.load(io.open('.v71ext/vecinos_%s.json' % i, encoding='utf-8'))
