# -*- coding: utf-8 -*-
"""Fase ciega de la 70, sin git: el grafo de HOY con las 20 de la tanda quitadas es, byte a byte, el grafo que barri
en la 68? Reconstruye el fichero como lo escribe src/comun.py escribir_jsonl (json.dumps con sort_keys y
ensure_ascii=False, una fila por linea, salto final) y compara su sha1 con la huella de dataset/nodos.jsonl en
.v68aud/huellas_al_barrer.txt. Antes comprueba que esa reconstruccion reproduce el fichero de hoy tal cual.
...
huella = [l.split(' *')[0] for l in io.open('.v68aud/huellas_al_barrer.txt', encoding='utf-8') if l.strip().endswith('*dataset/nodos.jsonl')][0]
...
print('(a) sin las 20 filas, sha1 igual a la huella de mi barrido de la 68: %s' % ('SI' if hashlib.sha1(ser(resto)).hexdigest() == huella else 'NO'))
tocados = 0; limpio = []
for f in resto:
    g = {}; t = False
    for k, v in f.items():
