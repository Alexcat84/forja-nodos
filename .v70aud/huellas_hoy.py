# -*- coding: utf-8 -*-
"""Fase ciega de la 70, sin git: las 50 huellas que tome al lanzar mi barrido de la 68 (.v68aud/huellas_al_barrer.txt:
las 47 fichas de la bandeja de Grove, las dos filas de cap_04 en _insertados y el grafo) contra los ficheros de HOY.
Una ficha de la bandeja que ya no esta en su sitio se busca en cuarentena/_insertados/grove_high_output/ con su mismo
nombre (D.31 la mueve alli al insertarla). El grafo no se compara aqui: lo mide .v70aud/grafo_sin_tanda.py. Solo lee."""
...
for l in io.open('.v68aud/huellas_al_barrer.txt', encoding='utf-8'):
    h, r = l.strip().split(' *', 1)
...
print('huellas: %d | por estado: %s | suma: %d' % (sum(c.values()), dict(c), sum(c.values())))
print('movidas a _insertados: %d | son las 20 de .v68aud/los20.txt: %s | fuera de ellas: %s' % (
