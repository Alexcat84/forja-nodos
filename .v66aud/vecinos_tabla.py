# Fase ciega de la 66: lee los .v66aud/vecinos_<id>.json que escribio .v66aud/barrido_uno.py y publica
...
orden = ['variar_frecuencia_inspeccion_nivel_calidad', 'simplificar_trabajo_reducir_numero_pasos'] + cap04
filas = []; pares = collections.OrderedDict(); falta = []
print('(1) candidato | poblacion | vecinos | en grafo | en bandeja')
for i in orden:
    f = '.v66aud/vecinos_%s.json' % i
...
for (a, b), lados in pares.items():
    tipo = 'cap04-cap04' if a in dentro and b in dentro else 'con fuera'
    print('    %-54s ~ %-54s %-11s %s' % (a, b, tipo, 'los dos' if len(lados) == 2 else 'solo ' + list(lados)[0]))
c = collections.Counter('cap04-cap04' if a in dentro and b in dentro else 'con fuera' for a, b in pares)
print('pares sin orden: %d | %s' % (len(pares), dict(c)))
