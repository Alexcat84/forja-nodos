# -*- coding: utf-8 -*-
"""Corre UN `python forja.py insertar` de la vuelta 70, filas 1 a 20 de .v69ext/orden.txt (COPIA de .v68ext/insertar.py, que era
COPIA de la 67), y espera a que vuelva.

    python .v70ext/insertar.py <fila> <id>

Lo que cambia de la copia, y se dice: las lineas --veredicto se copian TAL CUAL de su bloque `## <id>` de
.v68ext/veredictos_listos.txt (encargo de la 70, TAREA 3.1), con las correcciones de la relectura conjunta de la 69;
las lineas que empiezan por '#' dentro del bloque son las viejas que la correccion declarada dejo encima como comentario
y NO se pasan. Escribe .v70ext/insertar_<fila>_<id>.txt con el comando, la salida entera, el codigo y el reloj,
y al final el marcador .v70ext/insertar_<fila>_<id>.fin. No toca nada mas.
"""
import io, subprocess, sys, time

fila, cid = sys.argv[1], sys.argv[2]
lineas, dentro = [], False
for l in io.open('.v68ext/veredictos_listos.txt', encoding='utf-8').read().split('\n'):
    if l.startswith('## '):
...
for l in lineas:
    cmd += ['--veredicto', l]
base = '.v70ext/insertar_%s_%s' % (fila, cid)
t0 = time.time()
