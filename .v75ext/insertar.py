...
lineas, dentro = [], False
for l in io.open('.v73ext/veredictos_listos.txt', encoding='utf-8').read().split('\n'):
    if l.startswith('## '):
...
for l in lineas:
    cmd += ['--veredicto', l]
base = '.v75ext/insertar_%s_%s' % (fila, cid)
t0 = time.time()
