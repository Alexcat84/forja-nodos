...
nuevos=[l.strip() for l in io.open('.barrido_v18/nuevos.txt',encoding='utf-8') if l.strip()]
lineas=sorted(glob.glob('.barrido_v18/*.line'))
grafo=[l for l in io.open('dataset/nodos.jsonl',encoding='utf-8') if l.strip()]
out=io.open('.barrido_v18/BARRIDO_NUEVOS.txt','w',encoding='utf-8')
for f in nuevos:
    ...
    io.open('.barrido_v18/pob.jsonl','w',encoding='utf-8').writelines(pob)
    env=dict(os.environ); env['FORJA_DATASET']='.barrido_v18/pob.jsonl'
